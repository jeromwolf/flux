...
사용법:
    from flux.logging import setup_logging, get_logger
...
class TextFormatter(logging.Formatter):
    ...
    def __init__(self):
        super().__init__(
            fmt="[%(asctime)s KST] [%(levelname)s] [%(name)s] %(message)s",
            datefmt="%Y-%m-%d %H:%M:%S",
        )
...
def get_logger(name: str) -> logging.Logger:
    ...
    Args:
        name: 모듈 이름 (예: "config", "engine", "resilience")

    Returns:
        flux.{name} 로거
    """
    return logging.getLogger(f"flux.{name}")
