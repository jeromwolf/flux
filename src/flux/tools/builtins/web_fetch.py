...
def main(url, max_chars=3000):
    ...
    try:
        ...
        pinned_hosts = {parsed.hostname: resolved_ip}

        # DNS 핀닝: 호스트명을 해석된 IP로 교체하여 DNS 리바인딩 방지
        pinned_url = url.replace(f"://{parsed.hostname}", f"://{resolved_ip}", 1)

        session = requests.Session()
        response = session.get(
            pinned_url, headers=headers, timeout=10,
            allow_redirects=False, stream=True, verify=parsed.scheme == "https",
        )
        ...
        while response.is_redirect and redirects < 5:
            ...
            if rp.hostname and rp.hostname not in pinned_hosts:
                redirect_ip = _resolve_hostname(rp.hostname)
                if not redirect_ip:
                    return "Error: 리다이렉트 대상이 내부 네트워크 주소입니다."
                pinned_hosts[rp.hostname] = redirect_ip

            # 리다이렉트 URL도 핀닝
            rhost = rp.hostname or parsed.hostname
            if rhost in pinned_hosts:
                redirect_url = redirect_url.replace(
                    f"://{rhost}", f"://{pinned_hosts[rhost]}", 1
                )
                headers["Host"] = rhost

            response = session.get(
                redirect_url, headers=headers, timeout=10,
                allow_redirects=False, stream=True, verify=rp.scheme == "https" if rp.scheme else parsed.scheme == "https",
            )
            redirects += 1
